...
def get_sh_stocklist():
    ...
    for i in range(1, 96):  # 96页
        ...
        jQuery112408013832830150123_1612441015559&pn='+str(i)+'&pz=20&po=1&np=1&ut=\
        bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=m:1+t:2,m:1+t:23&\
        fields=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,f15,f16,f17,f18,f20,f21,f23,f24,f25,\
        f22,f11,f62,f128,f136,f115,f152&_=1612441015560'
...
def new_sh_stock_jud(csvfile):
    ...
    for i in idx:
        if i not in idx:
            ...
            ddata.html?code=0'+str(i)+'&start=20060101&end=20210204&\
...
def save_sh_db(idx):
    ...
    for code in idx:
        try:
            data = pd.read_csv(
                r'/Users/lmy/Desktop/findata/data_sh/'
                + str(code) + '.csv', encoding="gbk")
        except FileNotFoundError:
            print('正在获取股票%s数据' % code)
            url = 'http://quotes.money.163.com/service/ch\
            ddata.html?code=0'+str(code)+'&start=20060101&end=20210204&\
            fields=TCLOSE;HIGH;LOW;TOPEN;LCLOSE;CHG;PCHG;TURNOVER;\
            VOTURNOVER;VATURNOVER;TCAP;MCAP'   # TODO:每日更新日期
    ...
    # 创建数据表，如果数据表已经存在，会跳过继续执行下面的步骤print('创建数据表stock_%s'% fileName[0:6])
        sqlSentence3 = "create table if not exists stock_%s" % str(code) + "(日期 date, 股票代码 \
        VARCHAR(10), 名称 VARCHAR(10), 收盘价 float,\
