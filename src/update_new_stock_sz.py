...
def get_sz_stocklist():
    ticker = pd.DataFrame(columns={})
    for i in range(1, 124):  # 124页
        url = 'http://19.push2.eastmoney.com/api/qt/c\
        list/get?cb=jQuery112405729827281650932_16124507\
        09581&pn='+str(i)+'&pz=20&po=1&np=1&ut=bd1d9\
        ddb04089700cf9c27f6f7426281&fltt=2&invt=2&\
        fid=f3&fs=m:0+t:6,m:0+t:13,m:0+t:80&fields=\
        f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,\
        f15,f16,f17,f18,f20,f21,f23,f24,f25,f22,f11,\
        f62,f128,f136,f115,f152&_=1612450709582'
...
def new_sz_stock_jud(csvfile):
    ticker = get_sz_stocklist()
    idx = list(ticker.iloc[:, 0])
    sl = pd.read_csv('sza_list.csv', header=None, dtype=str)
    filepath = r'/Users/lmy/Desktop/findata/data_sz/'
    save = sl.iloc[1:, 1]
    for i in idx:
        if i not in idx:
            print('正在获取股票%s数据' % i)
            url = 'http://quotes.money.163.com/service/ch\
            ddata.html?code=1'+str(i)+'&start=20060101&end=20210204&\
            fields=TCLOSE;HIGH;LOW;TOPEN;LCLOSE;CHG;PCHG;TURNOVER;\
            VOTURNOVER;VATURNOVER;TCAP;MCAP'   # TODO:每日更新日期
            urllib.request.urlretrieve(url, filepath+str(i)+'.csv')
            save_sz_db(i)
...
def save_sz_db(idx):
    ...
    for code in idx:
        try:
            data = pd.read_csv(
                r'/Users/lmy/Desktop/findata/data_sz/'
                + str(code) + '.csv', encoding="gbk")
        except FileNotFoundError:
            print('正在获取股票%s数据' % code)
            url = 'http://quotes.money.163.com/service/ch\
            ddata.html?code=1'+str(code)+'&start=20060101&end=20210204&\
            fields=TCLOSE;HIGH;LOW;TOPEN;LCLOSE;CHG;PCHG;TURNOVER;\
            VOTURNOVER;VATURNOVER;TCAP;MCAP'   # TODO:每日更新日期
    ...
    # 创建数据表，如果数据表已经存在，会跳过继续执行下面的步骤print('创建数据表stock_%s'% fileName[0:6])
        sqlSentence3 = "create table if not exists stock_%s" % str(code) + "(日期 date, 股票代码 VARCHAR(10), \
        名称 VARCHAR(10), 收盘价 float,\
        最高价 float, 最低价 float, 开盘价 float, \
        前收盘 float, 涨跌额 float, 涨跌幅 float, 换手率 float,\
